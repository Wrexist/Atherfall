...
| 72 | **Social.** **Emotes done**: Wave, Cheer and Sit, from the Emote button on touch or V / C / X on desktop, with a bubble above the head so they read from the high camera. Other players see them over the live channel. **Parties** (`docs/PARTIES.md`): the database (step 1) and invites in the game (step 2) are done. Signed-in players get a Party button (P on desktop) that lists nearby players to invite. Invites pop up at once (Join / No thanks, gone after 30 s or when they expire), and the panel covers leave, remove (leader only) and a "take party invites" switch. Still to do: party frames on the HUD (step 3), shared kill credit and XP (step 4), friends, and chat with moderation (ask first). | ◐ |
...
| 93 | P0 | **Auto-attack, like Eternal Hero.** Walk into reach of an enemy and the hero turns to it and swings, still moving; stand still in a fight and it keeps swinging. It picks the locked target first, else the nearest in reach in any direction. Reach is the sword plus a lunge, or a shorter range than manual aim for bow and staff. Backing away no longer lunges you back in. A setting (on by default). Tested in `tests/autoAttack.test.ts`. | ✅ |
